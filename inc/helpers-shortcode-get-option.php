...
if( !function_exists( 'unysonplus_get_option_typography_css' ) ) :
	/**
	 * Color Picker
	 */
	function unysonplus_get_option_typography_css( $field ) {
		$output = '';
...
if ( ! function_exists( 'unysonplus_get_option_star_rating' )) :
/**
 *  Star Rating
 */
	function unysonplus_get_option_star_rating( $rating ) {
...
if(! function_exists('unysonplus_get_options_background_css')) :
	function unysonplus_get_options_background_css($atts) {
		if(isset($atts['background'])) {
			...
			if (is_array($bg['image'])) { // Check if a custom background has been set
				...
				$color	= unysonplus_get_option_color_picker( $bg['color'] );
			...
			if( isset( $bg['overlay'] ) && ( $bg['overlay']['selected'] == 'yes' ) ) {
				if( !empty( unysonplus_get_option_color_picker( $bg['overlay']['yes']['color'] ) ) ) {
					$overlay = array();
					$direction = $bg['overlay']['yes']['direction'];
					$color = unysonplus_get_option_color_picker( $bg['overlay']['yes']['color'] );
					if( !empty( unysonplus_get_option_color_picker( $bg['overlay']['yes']['gradient']) ) ) {
						$gradient = unysonplus_get_option_color_picker( $bg['overlay']['yes']['gradient'] );
					}else{
						$gradient = $color;
					}
					$overlay[] = '&:before { ';
...
if( ! function_exists('unysonplus_get_option_image') ) :
/** NOT USED... To be deleted.
 *  Get the image from options
 */
	function unysonplus_get_option_image( $atts ) {
		...
		$class = array_merge( $class, unysonplus_get_option_advanced_class( $atts ) );
		$attr['class'] = join(' ', $class);
		$attr = array_merge( $attr, unysonplus_get_option_animate_attr( $atts ) );

		$attr['src'] = $atts['image']['url'];
		
		return fw_html_tag( 'img', $attr, true );
	}
endif;


if( !function_exists('unysonplus_get_option_color_picker') ):
	/**
	 * Get Color Picker
	 */
	function unysonplus_get_option_color_picker( $atts ) {
...
if( !function_exists('unysonplus_get_option_link') ):
	...
	function unysonplus_get_option_link( array $link, $content = NULL ) {
		if( !empty($link[$link['selected']]['href']) ) {
			...
			return unysonplus_html_tag('a', $link[$link['selected']], $content);
		}else{
			return $content;
		}
	}
...
if( !function_exists('unysonplus_get_option_advanced_class') ) :
	/**
	 * Get Advanced Classes
	 */
	function unysonplus_get_option_advanced_class( $atts ) {
		...
		if( isset($atts['border']) ) {
			if ( array_filter($atts['border']['side'])) 																				$advanced_class[] = unysonplus_get_options_box_border($atts['border']);
...
if( !function_exists('unysonplus_get_option_spacing_css_property') ) :
	/**
	 * Get Advanced Classes
	 */
	function unysonplus_get_option_spacing_css_property( $atts, $spacing, $property ) { 
		$output = NULL;
...
if( !function_exists('unysonplus_get_option_spacing_css') ) :
	/**
	 * Spacing Breakpoints CSS
	 */
	function unysonplus_get_option_spacing_css( $atts ) {
		$shortcode_atts = array();
		$wrapper = '';
		if( isset( $atts['shortcode'] ) && in_array( $atts['shortcode'], array( 'section', 'column') ) ) $wrapper = ' .wrap';
		global $post;
		
		if( (null !== unysonplus_get_option_spacing_css_property( $atts, 'mall', 'margin') ) || ( null !== unysonplus_get_option_spacing_css_property( $atts, 'pall', 'padding' )) ) {
			$shortcode_atts[] = '.' . $post->post_type . '-' . $post->post_name . ' .' . $atts['id'] . $wrapper . ' { ';
				if( null !== unysonplus_get_option_spacing_css_property( $atts, 'mall', 'margin' ) )			$shortcode_atts[]	= unysonplus_get_option_spacing_css_property( $atts, 'mall', 'margin' );
				if( null !== unysonplus_get_option_spacing_css_property( $atts, 'pall', 'padding' ) )		$shortcode_atts[]	= unysonplus_get_option_spacing_css_property( $atts, 'pall', 'padding' );
			$shortcode_atts[] = '}';
		}
		
		if( (null !== unysonplus_get_option_spacing_css_property( $atts, 'msm', 'margin') ) || ( null !== unysonplus_get_option_spacing_css_property( $atts, 'psm', 'padding' )) ) {
			$shortcode_atts[] = '@media (min-width: 576px) {';
				$shortcode_atts[] = '.' . $post->post_type . '-' . $post->post_name . ' .' . $atts['id'] . $wrapper . ' { ';
					if( null !== unysonplus_get_option_spacing_css_property( $atts, 'msm', 'margin' ) )		$shortcode_atts[]	= unysonplus_get_option_spacing_css_property( $atts, 'msm', 'margin' );
					if( null !== unysonplus_get_option_spacing_css_property( $atts, 'psm', 'padding' ) )		$shortcode_atts[]	= unysonplus_get_option_spacing_css_property( $atts, 'psm', 'padding' );
				$shortcode_atts[] = '}';
			$shortcode_atts[] = '}';
		}
		
		if( (null !== unysonplus_get_option_spacing_css_property( $atts, 'mmd', 'margin') ) || ( null !== unysonplus_get_option_spacing_css_property( $atts, 'pmd', 'padding' )) ) {
			$shortcode_atts[] = '@media (min-width: 768px) {';
				$shortcode_atts[] = '.' . $post->post_type . '-' . $post->post_name . ' .' . $atts['id'] . $wrapper . ' { ';
					if( null !== unysonplus_get_option_spacing_css_property( $atts, 'mmd', 'margin' ) )		$shortcode_atts[]	= unysonplus_get_option_spacing_css_property( $atts, 'mmd', 'margin' );
					if( null !== unysonplus_get_option_spacing_css_property( $atts, 'pmd', 'padding' ) )		$shortcode_atts[]	= unysonplus_get_option_spacing_css_property( $atts, 'pmd', 'padding' );
				$shortcode_atts[] = '}';
			$shortcode_atts[] = '}';
		}
		
		if( (null !== unysonplus_get_option_spacing_css_property( $atts, 'mlg', 'margin') ) || ( null !== unysonplus_get_option_spacing_css_property( $atts, 'plg', 'padding' )) ) {
			$shortcode_atts[] = '@media (min-width: 992px) {';
				$shortcode_atts[] = '.' . $post->post_type . '-' . $post->post_name . ' .' . $atts['id'] . $wrapper . ' { ';
					if( null !== unysonplus_get_option_spacing_css_property( $atts, 'mlg', 'margin' ) )		$shortcode_atts[]	= unysonplus_get_option_spacing_css_property( $atts, 'mlg', 'margin' );
					if( null !== unysonplus_get_option_spacing_css_property( $atts, 'plg', 'padding' ) )		$shortcode_atts[]	= unysonplus_get_option_spacing_css_property( $atts, 'plg', 'padding' );
				$shortcode_atts[] = '}';
			$shortcode_atts[] = '}';
		}
		
		if( (null !== unysonplus_get_option_spacing_css_property( $atts, 'mxl', 'margin') ) || ( null !== unysonplus_get_option_spacing_css_property( $atts, 'pxl', 'padding' )) ) {
			$shortcode_atts[] = '@media (min-width: 1200px) {';
				$shortcode_atts[] = '.' . $post->post_type . '-' . $post->post_name . ' .' . '-'.$atts['id'] . $wrapper . ' { ';
					if( null !== unysonplus_get_option_spacing_css_property( $atts, 'mxl', 'margin' ) )		$shortcode_atts[]	= unysonplus_get_option_spacing_css_property( $atts, 'mxl', 'margin' );
					if( null !== unysonplus_get_option_spacing_css_property( $atts, 'pxl', 'padding' ) )		$shortcode_atts[]	= unysonplus_get_option_spacing_css_property( $atts, 'pxl', 'padding' );
				$shortcode_atts[] = '}';
			$shortcode_atts[] = '}';
		}
		return $shortcode_atts;
...
if( !function_exists('unysonplus_get_option_enqueue_wow') ) :
	/**
	 * Enqueue WOW Animate Script
	 */
	function unysonplus_get_option_enqueue_wow( $atts ) {
...
if( !function_exists('unysonplus_get_option_animate_attr') ) :
	/**
	 * Add WOW Animate Attributes
	 */
	function unysonplus_get_option_animate_attr( $atts ) {
		$animate_attr = array();
		if( isset($atts['animate']) && !empty($atts['animate']['animation']))	{
...
if( !function_exists('unysonplus_get_option_border_width') ) :
	/**
	 * Get Border Width Values
	 */
	function unysonplus_get_option_border_width( $atts ) {
		$css = array();
		if( ! empty( $atts['border']['width'] ) ) {
			$css[] = 'border-width: rem(' . preg_replace( '/[^0-9]/', '', $atts['border']['width'] ) . 'px) !important;';
		}
		return $css;
	}
endif;
...
if(! function_exists('unysonplus_options_get_id')) :
/**
 * Get the ID
 */
function unysonplus_options_get_id($shortcode,$id,$custom_id) {
	if (!empty($custom_id)) : 
		return $custom_id;
	else :
...
if( !function_exists('unysonplus_options_get_shortcode_css') ) :
	/**
	 * Prints the CSS in the sass.php file
	 */
	function unysonplus_options_get_shortcode_css( $atts, $shortcode_atts ) {	
		$shortcode_style = '';
